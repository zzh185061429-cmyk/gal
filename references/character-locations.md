...
## 10. 实践补遗（幻璃镜 2026-09 移植验证）

把本系统移植进探案题材卡后补齐的四个通用扩展与一条工程教训：

- **随行 follows 字段**：角色配置级 `follows: 目标角色名`，位置与标记完全复制目标的最终结果（除非自身有在场覆盖）。用于贴身侍女、护卫、双人行——保证永不与主子分头。
- **案内/业务期同框 caseRole**：在办事件（案件/委托/任务）期间，职内角色按 `prob` 与标记（同案/顾问/随行/尾行）直接写到玩家当前场景。签按 `hash(事件号+角色id)` 预签，一事之内恒定；玩家回到自宅路径视为收工休整，同框皆散；无玩家场景记录不出同框。职内角色由此在剧情焦点期自动聚拢，"查案查到孤家寡人"即消。
- **礼制角色行**：皇帝、长官这类只该按身份行动的角色，只给驻场加一条简易作息，无池子。位置表 ≠ 偶遇机制——它只陈述事实，不制造邂逅。
- **无星期世界**：架空/古代设定若没有星期概念，核心日程禁用 weekday 语义，一律逐日节律 + 深夜归巢硬时段。
- **工程教训：先核对蓝本路径再搜源码。** 工作区可能同时存在多份同名蓝本项目（如 `src/租借男友` 与 `src/rent/租借男友`），搜错目录会误判"该系统在源码里不存在"，然后凭蒸馏文档自己脑补出劣化版。本篇的权威源码位置以 SKILL.md §14 双蓝本对照声明的路径为准。
