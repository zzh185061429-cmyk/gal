...
| 阶段 | 产出 | 验收门 | 验在哪 | 里程碑底稿 |
|---|---|---|---|---|
| S0 决策与骨架 | 开工三格 + 九决策点 + 模板骨架跑起来 | 空壳 React 应用在浏览器打开不白屏 | 本地 | —（模板即骨架） |
...
规则：**跨阶段不加功能**。阶段内发现上一阶段设计错了，回到那一阶段改，不堆兼容补丁。每阶段结束向驾驶员汇报：做了什么 / 怎么验证的 / 哪些还没验——`driver-accepted` 只能驾驶员给。
...
7. **跨阶段不加功能**；每阶段过验收门，向驾驶员汇报证据（做了什么/怎么验/没验什么）。
...
| 文件 | 内容 |
|---|---|
| `00-环境与开工准备.md` | 动工前必读：环境清单、脚手架来路、本地调试闭环、构建发布两法、酒馆侧安装清单、开工三格 |
