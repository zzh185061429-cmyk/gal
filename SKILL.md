...
- **视觉/审美方向指导**（构图层级、CJK 排版工艺、动效论点、界面美术风格）→ 不在本套件范围。本 skill 的价值集中在**功能性**问题：宿主生命周期时序、数据一致性、交付运维——这些坑无法从文档推理，只能从实战项目蒸馏。它提供视觉工作的**机制地基**（语义色板、z 层令牌、reduced-motion 兜底），不提供审美判断；美术方向请自行另配视觉设计参考。
