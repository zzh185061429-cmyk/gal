...
| 角色地点池 | 常驻有名 NPC ≥4 且找人/偶遇是玩法 | 种子必须同时含角色+日期+小时（缺一 → 全员齐跳或全天不变）；串门先过滤（主人不在家时访客不得出现）；只注入 WHERE 不注入 WHAT | `character-locations.md`（§10 补遗：随行 follows、案内同框、礼制行、无星期适配） |
| 职守排班与事件预签 | AI 需周期性主动送事件/案源/委托（玩家不提就不来的病） | 触发描述全是被动句，节奏句写硬也压不住惰性 → 前端排班 + 日期种子预签 + 环境事实化；有在办事件义务机械虚设，一条文字节奏规则都不用写 | `duty-schedule.md` |
...
| 新手教程系统 | 想让新玩家第一次进入就被教会、老玩家永不被打扰 | 教程钩子声明晚于依赖它的 effect → TDZ 全屏崩溃（错误边界兜住，症状看起来与教程无关）；另造教程 UI 是歧路 → 台词构造成 ScriptLine[] 注入原生播放管线；演示内容全预写零 API，清理按内容匹配防残留 | `newcomer-tutorial.md` |
...
| `character-locations.md` | 角色地点池：三层位置模型、空闲池加权随机五步修正链、串门校验、位置表 AI 注入；§10 幻璃镜移植补遗（随行 follows、案内同框、礼制行、无星期适配、蓝本路径陷阱） |
| `duty-schedule.md` | 职守排班与事件预签：设计公理、四态今日行、有案虚设、节令合流与浮动节日公式（寒食/社日/伏日）、身份边界纪律、验收分层 |
...
| `newcomer-tutorial.md` | 新手教程系统：选择门（聊天变量认领持久化）、演出式引擎（ScriptLine 注入原生管线 + 按钮高亮/锁定 + 翻页拦截与绕行道封堵）、面板实操带练（Q版小人气泡 + 真实 API 演示 + 预写零 API + 讲到哪亮到哪）、演示数据卫生、真坑清单 |
